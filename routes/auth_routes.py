...
def verify_token():
    ...
    if not result.get("success"):
        return (
            jsonify({"success": False, "error": result.get("error", "Invalid token")}),
            401,
        )
    ...
    return jsonify(
        {
            "success": True,
            "data": {"uid": uid, "email": result["data"].get("email"), "user": user},
        }
    )
...
def reset_password():
    ...
    if result:
        ...
        return (
            jsonify({"success": False, "error": "Failed to send password reset email"}),
            500,
        )
