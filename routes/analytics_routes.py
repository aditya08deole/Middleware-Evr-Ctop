    ...
    try:
        ...
        for device in devices:
            if device.get("last_sync_time"):
                sync_date = (
                    device["last_sync_time"][:10]
                    if isinstance(device["last_sync_time"], str)
                    else device["last_sync_time"].strftime("%Y-%m-%d")
                )
        ...
        return jsonify(
            {"success": True, "data": {"labels": sorted_dates, "values": sorted_values}}
        )
...
def get_success_failure_rate():
    ...
    try:
        ...
        if not fs:
            # Return sample data if Firestore not available
            return jsonify(
                {"success": True, "data": {"success": 75, "failure": 15, "pending": 10}}
            )
        ...
        for log in logs:
            if log.get("created_at"):
                log_date = (
                    ...
                    else log["created_at"].strftime("%Y-%m-%d")
                )
