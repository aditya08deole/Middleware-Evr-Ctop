...
class FirestoreService:
    ...
    def __init__(self):
        ...
        if not credentials_path or not os.path.exists(credentials_path):
            if not hasattr(self, "_warned"):
                print(
                    "WARNING: Firebase credentials not found. Set FIREBASE_CREDENTIALS_PATH to use Firestore."
                )
                self._warned = True
            self._db = None
            return
            ...
            if current_time - self._devices_cache["timestamp"] < self.CACHE_TTL_SECONDS:
                ...
                if filters and "is_active" in filters:
                    return [
                        ...
                        if d.get("is_active") == filters["is_active"]
                    ]
    ...
    def get_logs(
        self, device_id: str = None, log_type: str = None, limit: int = 100
    ) -> List[Dict]:
    ...
    def get_stats(self) -> Dict:
        ...
        try:
            ...
            total_processed = (
                self._db.collection("processed_data").count().get()[0][0].value
            )
