...
def process_device(device_id):
    ...
    try:
        ...
        if data_source == 'emqx' and not raw_data.get('feeds') and not emqx_service.is_connected(device_id):
            logger.warning(
        ...
        if backoff_seconds and device.last_ctop_attempt_time and \
                (now - device.last_ctop_attempt_time).total_seconds() < backoff_seconds:
            logger.debug(
        ...
        device.consecutive_failures = 0 if success_count > 0 else consecutive_failures + 1
        device.needs_attention = device.consecutive_failures >= CTOP_ATTENTION_THRESHOLD
        device.last_ctop_attempt_time = now
        if device.needs_attention and device.consecutive_failures == CTOP_ATTENTION_THRESHOLD:
            logger.warning(
...
def init_scheduler(app):
    ...
    try:
        with app.app_context():
            devices = Device.query.filter_by(is_active=True).all()
            if devices:
                emqx_service.subscribe_all_devices([d.to_dict_with_credentials() for d in devices])
    except Exception as e:
