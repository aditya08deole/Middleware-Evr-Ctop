...
def process_device(device_id, device_data):
    ...
    if not device_data.get("is_active", True):
        logger.info(
            f"Device {device_data.get('name')} (ID: {device_id}) is inactive, skipping"
        )
        return
    ...
    try:
        ...
        if data_source == "emqx":
            ...
            success, raw_data, error = thingspeak_service.fetch_data(
                device_id, device_data
            )

        if not success:
            logger.error(
                f"Failed to fetch data for device {device_data.get('name')}: {error}"
            )
            ...
            local_device_store.increment_device_stats(
                device_id, fetch_success=False, send_success=False
            )
            return
        ...
        if latest_reading_time:
            ...
            logger.info(
                f"Fetched fresh data for device {device_data.get('name')} (reading time: {latest_reading_time.isoformat()})"
            )
            ...
            if stored_reading_time:
                try:
                    stored_dt = datetime.fromisoformat(
                        str(stored_reading_time).replace("Z", "+00:00")
                    )
                except (ValueError, AttributeError):
                    stored_dt = None
            ...
            if stored_dt is None:
                logger.info(
                    f"Device {device_data.get('name')} has no prior reading. Marking INACTIVE."
                )
                local_device_store.update_device_fields(
                    device_id,
                    {
            ...
            logger.debug(
                f"Device {device_data.get('name')} idle this tick (last reading {age_minutes:.1f} min ago)."
            )
            local_device_store.update_device_fields(
                device_id, {"last_sync_time": now_utc.isoformat()}
            )
            return
        # ── END STALENESS GUARD ────────────────────────────────────────────────
        ...
        success, processed_data, error = preprocess_service.preprocess_data(
            device_id, raw_data, device_data=device_data
        )

        if not success:
            logger.error(
                f"Failed to preprocess data for device {device_data.get('name')}: {error}"
            )
            local_device_store.update_device_fields(
                device_id,
                {
            ...
            local_device_store.increment_device_stats(
                device_id, fetch_success=True, send_success=False
            )
            return
        ...
        if not processed_data:
            ...
            logger.debug(
                f"No new entries to process for device {device_data.get('name')}"
            )
            local_device_store.update_device_fields(
                device_id,
                {
                    "last_reading_time": (
                        latest_reading_time.isoformat() if latest_reading_time else None
                    ),
                    "last_sync_time": datetime.utcnow().isoformat(),
                },
            )
        ...
        transformed_data = preprocess_service.transform_to_ctop_format(
            device_id, processed_data, device_data=device_data
        )
        ...
        for i, entry in enumerate(processed_data):
            ...
            if last_entry_id > 0 and entry_id <= last_entry_id:
                logger.debug(
                    f"Skipping old entry {entry_id} for device {device_id} (last processed: {last_entry_id})"
                )
                continue

            logger.info(
                f"NEW DATA detected for device {device_id}: entry_id {entry_id} > last {last_entry_id}"
            )
        ...
        if not new_transformed_data:
            ...
            # left it.
            logger.debug(
                f"No new data for device {device_id} (last entry: {last_entry_id})"
            )
            local_device_store.update_device_fields(
                device_id,
                {
        ...
        if backoff_seconds:
            ...
            if last_attempt_str:
                try:
                    last_attempt = datetime.fromisoformat(
                        str(last_attempt_str).replace("Z", "+00:00")
                    )
                except (ValueError, TypeError):
                    last_attempt = None
            ...
            if last_attempt and (now - last_attempt).total_seconds() < backoff_seconds:
                ...
                if data_source == "emqx":
                    emqx_service.rollback_fetch(device_id)
                local_device_store.update_device_fields(
                    device_id, {"last_sync_time": now.isoformat()}
                )
                return
        ...
        logger.info(
            f"Sent {success_count}/{len(new_transformed_data)} NEW payloads to CTOP for device {device_data.get('name')}"
        )
        ...
        local_device_store.increment_device_stats(
            device_id, fetch_success=True, send_success=success_count > 0
        )
...
def scheduled_job():
    ...
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            future_to_device = {
                executor.submit(process_device_safe, device["id"], device): device.get(
                    "name", "Unknown"
                )
            ...
            for future in concurrent.futures.as_completed(future_to_device):
                ...
                try:
                    ...
                    logger.error(
                        f"Parallel processing failed for device {device_name}: {str(e)}"
                    )
...
def init_scheduler(app):
    ...
    try:
        scheduler.start()
        logger.info(
            f"Firestore Scheduler initialized with {interval_seconds} second interval"
        )
...
def reschedule_job(seconds):
    ...
    try:
        scheduler.reschedule_job(
            "iot_data_pipeline_firestore", trigger=IntervalTrigger(seconds=seconds)
        )
        logger.info(
            f"Rescheduled Firestore IoT Data Pipeline to {seconds} second interval"
        )
        return True
    except Exception as e:
