...
def test_new_entry_id_is_recorded_as_a_new_point():
    ...
    store.record('dev1', '1001', '2026-09-10T09:51:14+00:00', {'water_level': 50.0})  # retry, same entry
    store.record('dev1', '1002', '2026-09-10T09:52:21+00:00', {'water_level': 51.6})  # genuinely new
...
def test_non_numeric_values_are_dropped_and_empty_reading_not_recorded():
    store = ReadingHistoryStore()
    store.record('dev1', '1', '2026-09-10T09:50:00+00:00', {'status': 'ok', 'note': None})
    assert store.get_history('dev1') == []

    store.record('dev1', '2', '2026-09-10T09:51:00+00:00', {'status': 'ok', 'water_level': 12.5})
    history = store.get_history('dev1')
    assert len(history) == 1
