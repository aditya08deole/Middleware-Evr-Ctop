...
def test_process_device_routes_emqx_devices_through_emqx_service(app):
    ...
    with app.app_context(), mock.patch.object(
        scheduler_module, "emqx_service", mock_emqx
    ), mock.patch.object(
        scheduler_module, "thingspeak_service", mock_thingspeak
    ), mock.patch.object(
        scheduler_module, "preprocess_service", mock_preprocess
    ), mock.patch.object(
        scheduler_module, "ctop_service", mock_ctop
    ):
...
def test_process_device_safe_skips_concurrent_call_for_same_device(app):
    with app.app_context():
        device = _make_device(
            data_source="thingspeak", emqx_broker_url=None, emqx_topic=None
        )
        device_id = device.id
...
def test_process_device_safe_pushes_app_context_for_bare_thread_call(app):
    ...
    with mock.patch.object(scheduler_module, "_app", app), mock.patch.object(
        scheduler_module, "thingspeak_service", mock_thingspeak
    ), mock.patch.object(
        scheduler_module, "preprocess_service", mock.MagicMock()
    ), mock.patch.object(
        scheduler_module, "ctop_service", mock.MagicMock()
    ), mock.patch.object(
        scheduler_module, "emqx_service", mock.MagicMock()
    ):
