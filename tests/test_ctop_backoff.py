...
class TestFirestoreSchedulerBackoff:
    def _run(self, device_data, send_result):
        ...
        mock_thingspeak.fetch_data.return_value = (
            True,
            {"feeds": [_fresh_feed()]},
            None,
        )
        ...
        with patch.object(sf, "thingspeak_service", mock_thingspeak), patch.object(
            sf, "preprocess_service", mock_preprocess
        ), patch.object(sf, "ctop_service", mock_ctop), patch.object(
            sf, "local_device_store", mock_local_store
        ):
    ...
    def test_success_resets_consecutive_failures(self):
        ...
        mock_ctop, mock_local_store = self._run(
        ...
        tracking_call = next(
            c
            for c in mock_local_store.update_device_fields.call_args_list
    ...
    def test_failure_increments_and_sets_needs_attention_at_threshold(self):
        device_data = _base_device_data(
            consecutive_failures=4
        )  # one below threshold (5)
        mock_ctop, mock_local_store = self._run(
            device_data, {"success": False, "error": "boom"}
        )
    ...
    def test_backoff_skips_send_within_cooldown_window(self):
        ...
        device_data = _base_device_data(
            consecutive_failures=10, last_ctop_attempt_time=recent_attempt
        )
        mock_ctop, mock_local_store = self._run(
            device_data, {"success": True, "error": None}
        )
    ...
    def test_backoff_allows_send_after_cooldown_elapses(self):
        ...
        device_data = _base_device_data(
            consecutive_failures=10, last_ctop_attempt_time=old_attempt
        )
        mock_ctop, mock_local_store = self._run(
            device_data, {"success": True, "error": None}
        )
    ...
    def test_low_failure_count_never_backs_off(self):
        ...
        device_data = _base_device_data(
            consecutive_failures=2, last_ctop_attempt_time=recent_attempt
        )
        mock_ctop, mock_local_store = self._run(
            device_data, {"success": True, "error": None}
        )
...
class TestSqliteSchedulerBackoff:
    def _run(self, app, device_id, send_result):
        ...
        with app.app_context(), patch.object(
            sqlite_scheduler, "thingspeak_service", mock_thingspeak
        ), patch.object(
            sqlite_scheduler, "preprocess_service", mock_preprocess
        ), patch.object(
            sqlite_scheduler, "ctop_service", mock_ctop
        ), patch.object(
            sqlite_scheduler, "emqx_service", MagicMock()
        ):
