...
def test_average_filter_smooths_across_separate_calls():
    ...
    _, entries1, _ = service.preprocess_data(device_id, {'feeds': [_feed(1, 10)]}, device_data=device_data)
    _, entries2, _ = service.preprocess_data(device_id, {'feeds': [_feed(2, 20)]}, device_data=device_data)
    _, entries3, _ = service.preprocess_data(device_id, {'feeds': [_feed(3, 30)]}, device_data=device_data)
...
def test_median_filter_smooths_across_separate_calls():
    ...
    for i, value in enumerate([10, 100, 20], start=1):
        _, entries, _ = service.preprocess_data(device_id, {'feeds': [_feed(i, value)]}, device_data=device_data)
...
def test_filter_history_is_isolated_per_device():
    ...
    service.preprocess_data('device-a', {'feeds': [_feed(1, 1000)]}, device_data=device_data)
    _, entries_b, _ = service.preprocess_data('device-b', {'feeds': [_feed(1, 5)]}, device_data=device_data)
