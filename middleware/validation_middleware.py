    ...
    def decorated_function(*args, **kwargs):
        ...
        if data_source == "thingspeak":
            ...
            if not str(data["channel_id"]).isdigit():
                return (
                    jsonify({"success": False, "error": "Channel ID must be numeric"}),
                    400,
                )
        ...
        if not is_valid_url(data["ctop_url_1"]):
            return (
                jsonify({"success": False, "error": "Invalid ctop_url_1 format"}),
                400,
            )
        ...
        if data.get("ctop_url_2") and not is_valid_url(data["ctop_url_2"]):
            return (
                jsonify({"success": False, "error": "Invalid ctop_url_2 format"}),
                400,
            )
        ...
        if data.get("latitude") is not None:
            try:
                ...
                return (
                    jsonify({"success": False, "error": "Invalid latitude format"}),
                    400,
                )
        ...
        if data.get("longitude") is not None:
            try:
                ...
                return (
                    jsonify({"success": False, "error": "Invalid longitude format"}),
                    400,
                )
...
def validate_pagination(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if per_page < 1 or per_page > 100:
            return (
                jsonify(
                    {"success": False, "error": "Per page must be between 1 and 100"}
                ),
                400,
            )
